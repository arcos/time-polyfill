...
- arcos fork: https://github.com/arcos/time-polyfill
*/
...
(function() {
  ...
  (function($) {
    $.fn.inputTime = function() {
      var decrement, increment, makeTimeDisplayString, makeTimeString, readTime, stepNormalize, $this;
      ...
      makeTimeString = function(time_obj) {
        ...
        if (time_obj.getSeconds() > 0 && !$this.data('hide-seconds')) {
          t_arr.push(':');
      ...
      $(this).filter('input[type="time"]').each(function() {
        var $hiddenField, $this, $timeField, btnContainer, className, downBtn, halfHeight, hiddenField, max, min, step, style, timeField, upBtn, value, id, is_disabled;
        $this = $(this);
        value = $this.attr('value');
        min = $this.attr('min');
        is_disabled = $this.attr('disabled') === "disabled";
        max = $this.attr('max');
        step = $this.attr('step');
